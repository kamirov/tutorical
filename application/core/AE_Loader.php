...
//            $vars['notices_count'] = 90;
        ...
        if (isset($vars['extra_make_tutor_request']))
        {
            // Need to call this again because is_modal has been set to false
            $vars['extra_make_request'] = $this->view('requests/make', $vars, TRUE);
        }
 ...
 //           var_dump($this->ci->session->all_userdata());
    ...
    public function get_search_form()
    {
        ...
        if ($data['readable_location'])
            $data['readable_location'] = html_entity_decode($data['readable_location']['readable']);
//            $data['readable_location'] = ae_urldecode($data['readable_location']['readable']);
        else
...
/*
        ...
        if (!empty($this->subjects)) {
            ...
            var_dump($data['subject_options']);
        }
*/      
//        $data['subject_options'] = combine_subarrays($this->subjects, 'name');;
//        var_dump($data['subject_options']);
