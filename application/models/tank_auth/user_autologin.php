...
class User_Autologin extends CI_Model
{
	...
	function get($user_id, $key)
	{
		...
		$this->db->select($this->users_table_name.'.avatar_path');
		$this->db->select($this->users_table_name.'.display_name');
		$this->db->select($this->users_table_name.'.first_name');
		$this->db->select($this->users_table_name.'.last_name');
