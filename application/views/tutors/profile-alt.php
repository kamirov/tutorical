	...
	<div class="boxes dropdown-panel">
	      ...
	      addthis_share = 
	      {
	        passthrough : 
	        {
	              twitter: 
	              {
	                  <? if ($name == $tutor['main_subject']['name']): ?>
	                      text: "Found a great <?= $name ?> tutor in <?= $tutor['city'] ?>, <?= $tutor['country'] ?> - " + <?= json_encode($tutor['display_name']) ?> + " | "
	                  <? else: ?>
...
<section id="tutor-profile" class="cf pages containers profiles">
	<header class="persistent-headers profile-header" id="persistent-profile-header" data-appear-offset="280">
		...
		<div class="containers">
			...
			<span id="header-contact-user-button" class="buttons color-3-buttons">Contact <?= $tutor['display_name'] ?></span>
			<img alt="Tutor photo" id="profile-header-avatar" src="<?= $tutor['avatar_url'] ?>">
			<h1 href="#top" id="profile-header-name"><?= $tutor['display_name'] ?></h1>
		</div>
	</header>

	<section id="user-intro" class="profile-secs col-1">
			...
			<div class="price-conts">
				<div class="prices">
					...
					<? elseif ($tutor['price_type'] == 'free'): ?>
							...
							<? if ($tutor['reason']): ?>
								<span class="reason-for-frees"><span class="tiny-aftertext">(<span class="same-page-links no-pointer downwards-qtipped" title="<?= nl2br($tutor['reason']) ?>">why?</span>)</span></span>
							<? endif; ?>
					<? endif; ?>
					</span>
				</div>
	...
	<section id="user-details" class="profile-secs col-1">
	...
	<section id="user-contact" class="profile-secs"> 	
		<span class="buttons color-3-buttons" id="user-contact-button">
			<span id="user-contact-button-content">Contact <?= $tutor['display_name'] ?></span>
		</span>
...
-->						<div class="form-elements">
		...
		<div id="under-contact-links-cont">
			<span title="Favourited tutors show up on your Tutors page. Favourite a tutor to make them easy to find later." class="under-contact-links dropdown-padding link-like" id="update-favourites" data-favourited="<?= $favourited ?>"><?= ($favourited ? 'Unfavourite' : 'Favourite') ?></span><img src="<?= base_url('assets/images/'.LOADER_LIGHT) ?>" class="ajax-loaders" id="favourite-loader"> | 
			<span href="#" data-dropdown="#dropdown-invite-to-tutor-request" class="under-contact-links link-like" title="Invite a tutor to one of your requests to indicate that you'd like them to apply.">Invite</span>
		...
		<section id="tutor-links" class="profile-secs">
			...
			<div id="links-item-cont" data-type="link" class="item-conts"> 
				<? foreach($tutor['links'] as $item): ?>
					<div class="link-items block-items">
	...
	<? if (!empty($tutor['reviews'])): ?>
	<section id="user-reviews-sec" class="profile-secs col-1 <? if (!empty($tutor['external_reviews'])) { echo 'external-reviews-beneath'; } ?>">
		<header itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
			...
			?>
				<div class="user-reviews" data-review-id="<?= $review['id'] ?>">
					<form>
						...
						<div class="review-thread-conts" data-comment-id="<?= $review['comment']['id'] ?>">
								...
								<li class="review-thread-items">
									<div class="review-thread-item-text-conts">
										...
										<div class="review-thread-item-authors">
											...
											<? if ($review['comment']['contact_id'] == DELETED_ID): ?>
												<span class="review-thread-item-author-names deleted" title="This person deleted their account"><?= $review['comment']['display_name'] ?></span>
											<? else: ?>
	...
	<section id="user-external-reviews-sec" class="profile-secs col-1">
		<header>
			<span class="same-page-links no-pointer left-qtipped" id="external-reviews-what" title="These reviews were posted on other websites and were submitted by the tutor. We encourage you to click 'Visit Site' under the reviews to see the original site and to determine the credibility of each one.">What's this?</span>
