...
<section class="find-bars containers cf">
	...
	<div class="sort-by-cont">
		...
		<? if ($groups == 'requests'): ?>
			<span class="search-types active">Requests</span> | <?= anchor("find/tutors/$location_query/$subject_query", "Tutors", 'class="search-types" title="See '.($readable_subject ? $readable_subject.' ' : '').'tutors within '.$readable_distance.' of '.$readable_location.'"') ?>
		<? else: ?>
			<?= anchor("find/requests/$location_query/$subject_query", "Requests", 'class="search-types" title="See '.($readable_subject ? $readable_subject.' ' : '').'requests within '.$readable_distance.' of '.$readable_location.'"') ?> | <span class="search-types active">Tutors</span>
		<? endif; ?>
		</span>
		...
		<span class="sort-by-description-text">
			...
			<? endif; ?>			
		| Showing: 
		<a href="#" data-dropdown="#dropdown-sorts" class="sort-options"><?= $readable_sort ?> <span class="sort-arrows">↓</span></a></span>		
		...
		<span class="sort-by-description-text">within <a href="#" data-dropdown="#dropdown-distances" class="sort-options"><?= $distance ?> <span class="sort-arrows">↓</span></a></span>
		...
		<span class="sort-by-description-text"><a href="#" data-dropdown="#dropdown-units" class="sort-options"><?= $readable_units ?> <span class="sort-arrows">↓</span></a></span>
...
<section id="find-results" class="cf containers">
	...
	<?
		...
		<table id="find-item-results">
	<?
			foreach ($items as $tutor):
	?>
				<tr class="tutor-results">
					<td class="avatar-and-marker-conts">
						<img alt="Map Marker" class="map-markers" src="<?= $tutor['marker_url']; ?>">
						<div class="avatar-and-profile-links">
							<?= anchor('tutors/'.$tutor['username'], '<img alt="Avatar" class="avatars" src="'.$tutor['avatar_url'].'" />') ?>
							<?= anchor('tutors/'.$tutor['username'], 'View Profile', 'class="view-profile-links"') ?>
						</div>
					</td>
					<td class="tutor-result-items">
						<? 
...
<h1 id="page-heading" class="none-found-heading">
	...
	<div id="page-content" class="text-content">
		<p>But you might find some if you <a href="#" data-reveal="request-modal">make a Tutor Request</a>. It's free and it'll let tutors find you!</p>
	</div>
<? else: ?>
<h1 id="page-heading" class="none-found-heading">
